...
const INITIAL_UI = { scan: false, code: '', qty: '' };
...
export default function App() {
  ...
  const [db, setDb] = useState(INITIAL_PRODUCTS);

  const metrics = useMemo(() => {
    const stockTotal = db.reduce((acc, product) => acc + product.stock, 0);
    const alertsCount = db.filter((product) => product.stock <= product.min).length;

    return {
      stockTotal,
      productsCount: db.length,
      alertsCount,
      upcomingCount: 8,
    };
  }, [db]);

  const updateUi = (partialState) => {
    setUi((previousState) => ({ ...previousState, ...partialState }));
  };

  const handleOp = (type) => {
    const code = ui.code.trim();
    const qty = Number.parseInt(ui.qty, 10);
    ...
    setDb((previousDb) =>
      previousDb.map((product) => {
        if (product.cb !== code) {
          return product;
        }

        if (type === 'OUT' && product.stock - qty < 0) {
          window.alert('CU-03: Stock insuficiente');
          return product;
        }

        return {
          ...product,
          stock: type === 'IN' ? product.stock + qty : product.stock - qty,
        };
      }),
    );
  ...
  const handleScanToggle = () => {
    updateUi({ scan: !ui.scan });
  };

  const handleScanUpdate = (_, result) => {
    if (!result) {
      return;
    }

    setUi((previousState) => ({ ...previousState, code: result.text, scan: false }));
  };
    ...
    <div className="flex min-h-screen bg-[#09090b] text-[#a1a1aa] font-sans">
      <aside className="w-64 border-r border-[#1e1e2e] p-6 hidden lg:block bg-[#09090b]">
        <div className="flex items-center gap-2 mb-10 text-white font-bold tracking-tighter">
          <div className="w-6 h-6 bg-red-600 rounded flex items-center justify-center italic text-xs">I</div>{' '}
          ITCV INVENTARIO
        </div>
        <nav className="space-y-1">
          {['Dashboard', 'Inventario', 'Movimientos', 'Reportes', 'Ajustes'].map((menuItem, index) => (
            <div
              key={menuItem}
              className={`p-2 rounded-md text-sm cursor-pointer transition ${
                index === 0 ? 'bg-[#18181b] text-white' : 'hover:bg-[#111] hover:text-white'
              }`}
            >
              {menuItem}
            </div>
      ...
      <main className="flex-1 p-6 lg:p-10 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-[#09090b] border border-[#1e1e2e] p-5 rounded-xl">
            <div className="flex justify-between items-start text-[#71717a] text-[10px] font-bold uppercase mb-2">
              Stock Total <span>📦</span>
            </div>
            <p className="text-2xl font-bold text-white">{metrics.stockTotal}</p>
          </div>
          <div className="bg-[#09090b] border border-[#1e1e2e] p-5 rounded-xl">
            <div className="flex justify-between items-start text-[#71717a] text-[10px] font-bold uppercase mb-2">
              Productos <span>📈</span>
            </div>
            <p className="text-2xl font-bold text-white">{metrics.productsCount}</p>
          </div>
          <div className="bg-[#09090b] border border-[#1e1e2e] p-5 rounded-xl border-red-900/30">
            <div className="flex justify-between items-start text-red-500 text-[10px] font-bold uppercase mb-2">
              Alertas <span>⚠️</span>
            </div>
            <p className="text-2xl font-bold text-white">{metrics.alertsCount}</p>
          </div>
          <div className="bg-[#09090b] border border-[#1e1e2e] p-5 rounded-xl">
            <div className="flex justify-between items-start text-purple-500 text-[10px] font-bold uppercase mb-2">
              Próximos <span>🕒</span>
            </div>
            <p className="text-2xl font-bold text-white">{metrics.upcomingCount}</p>
          </div>
        </div>
        ...
        <section className="bg-[#09090b] border border-[#1e1e2e] p-6 rounded-xl mb-8">
          <div className="flex flex-col md:flex-row gap-3 items-end">
            <div className="flex-1 w-full space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <input
                  value={ui.code}
                  onChange={(event) => updateUi({ code: event.target.value })}
            ...
            <div className="w-full md:w-24 space-y-2">
              ...
              <input
                type="number"
                value={ui.qty}
                onChange={(event) => updateUi({ qty: event.target.value })}
                placeholder="0"
            ...
            <div className="flex gap-2 w-full md:w-auto">
              <button
                onClick={() => handleOp('IN')}
                className="flex-1 md:w-28 bg-white text-black font-bold py-2 rounded-md text-sm hover:bg-zinc-200"
              >
                ENTRADA
              </button>
              <button
                onClick={() => handleOp('OUT')}
                className="flex-1 md:w-28 bg-[#18181b] border border-[#27272a] text-white font-bold py-2 rounded-md text-sm hover:text-red-500"
              >
                SALIDA
              </button>
            </div>
          ...
          {ui.scan && (
            <div className="mt-4 max-w-sm mx-auto rounded-xl overflow-hidden border border-red-600 shadow-2xl">
              <BarcodeScannerComponent onUpdate={handleScanUpdate} />
            </div>
          )}
        </section>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody>
                {db.map((product) => (
                  <tr
                    key={product.id}
                    className="border-b border-[#1e1e2e] hover:bg-[#18181b]/40 transition-colors text-white"
                  >
                    <td className="p-4">
                      <p className="font-bold">{product.name}</p>
                      <p className="text-[10px] text-[#71717a] font-mono">{product.cb}</p>
                    </td>
                    <td className="p-4 font-black text-lg">{product.stock}</td>
                    <td className="p-4 text-[#71717a]">{product.cad}</td>
                    <td className="p-4 text-right">
                      <span
                        className={`px-2 py-1 rounded-full text-[9px] font-black border ${
                          product.stock <= product.min
                            ? 'bg-red-500/10 text-red-500 border-red-500/20'
                            : 'bg-green-500/10 text-green-500 border-green-500/20'
                        }`}
                      >
                        {product.stock <= product.min ? '⚠ STOCK BAJO' : '✓ NORMAL'}
                      </span>
                    </td>
                  </tr>
                ))}
